...
p12 <- ggplot(pca_df, aes(x = PC1, y = PC2)) +
  ...
  stat_ellipse(aes(color = decision))
...
```{r}
build <- ggplot_build(p12)$data
pts <- build[[1]] %>% sf::st_as_sf(coords = c('x', 'y'))
ell <- build[[5]] %>% sf::st_as_sf(coords = c('x', 'y')) %>% concaveman::concaveman()

in_ell <- sf::st_filter(pts, ell) %>% 
  cbind(st_coordinates(.))
in_ell1 <- filter(in_ell, group == 1)
in_ell2 <- filter(in_ell, !group %in% c(1, 3))
in_ell3 <- filter(in_ell, !group %in% c(1, 2))

p12 +
  geom_point(data = in_ell1, aes(X, Y), color = 'red')
  geom_point(data = in_ell3, aes(X, Y), color = 'blue')
  # geom_point(data = in_ell2, aes(X, Y), color = 'green') +
```
