...
```{r}
cit_scr_df <- read_csv(here('_data/5a_societal_benefits/sbr_screened_results_240429.csv')) %>%
  mutate(title = str_to_title(title))

fulltext_df <- read_csv(here('_output/phase1_societal_benefits/fulltext_screened.csv')) %>%
...
scale_factor <- min(max(abs(pca_df$PC1)), max(abs(pca_df$PC2))) / 
                max(abs(sbr_faiss_pca$rotation[, 1:2]))
  
### dataframe for loadings; scale up by scaling_factor for plotting
pca_rot_df <- data.frame(sbr_faiss_pca$rotation %>% round(4) * scale_factor) %>%
...
ggplot(data = sample_df, aes(x = PC1, y = PC2)) +
...
sum(excl_test) / length(excl_test)

sum(todo_test) / length(todo_test)
