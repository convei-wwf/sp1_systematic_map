...
sbr_faiss_wide <- sbr_vs_faiss %>%
  filter(!is.na(query)) %>%
  select(-dist, -key, -query) %>%
  pivot_wider(names_from = 'criteria', values_from = 'dist_norm')
...
Having selected the best match for each criterion, we now see very little collinearity among criteria a-c (though lots between a and d).  We therefore probably don't need to worry about a PCA to reduce collinearity, and can just use scores for criteria a-c directly. That said, let's still check out a PCA:
...
Let's examine between PC 2 and 3:

```{r}
pca_hull_df <- pca_df %>%
  arrange(decision, PC3, PC2) %>%
  group_by(decision) %>%
  slice(chull(PC3, PC2))
...
p23 <- ggplot(pca_df, aes(x = PC3, y = PC2)) +
  geom_point(aes(color = decision)) +
  geom_segment(data = pca_rot_df, x = 0, y = 0, 
...
The screeplot shows that 90% of the variance is captured in the first four principal components.  We don't necessarily need the dimensionality reduction at this point, but removing the collinearity might be helpful for the binary logistic regression model.
...
Based on this, the binary logistic regression seems to do a good job of prediction.  However, we are more concerned about false negatives (rejecting a document that should be included) than false positives (including a document that should be excluded), and our dataset is a bit unbalanced (~20% inclusion) and the full dataset is likely to be far more unbalanced, so these "success metrics" are likely driven by high true negative rates.  Let's pull info out of the `blr_last`, and plot a ROC and confusion matrix to see if there's a good threshold for probability that drops true negatives but doesn't drop many true positives...

```{r}
blr_check_df <- blr_last$.predictions[[1]]

blr_check_df %>% select(.pred_class, decision) %>% table()

blr_check_df %>% filter(decision == 'include') %>% arrange(.pred_include)
```

From this rough cut, no false positives, but a high false negative rate (7 out of 11 "includes" classified as "exclude").  Not great!  The `.pred_include` column shows that false negatives start at only 9.6%, with many more in the 10-20% range.  

```{r}
roc_curve(blr_check_df, truth = decision, .pred_exclude) %>%
  autoplot()
```
...
```{r}
false_neg_indices <- blr_check_df %>% filter(decision == 'include' & .pred_class == 'exclude') %>% pull(.row)

sbr_faiss_wide %>%
  mutate(across(where(is.numeric), ~round(.x, 3))) %>%
  slice(false_neg_indices) %>%
  pivot_longer(names_to = 'query', values_to = 'val', -c(author, title, decision, tags)) %>%
