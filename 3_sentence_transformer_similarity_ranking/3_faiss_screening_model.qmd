...
rownames(corr_mtx) <- colnames(corr_mtx) <- LETTERS[1:nrow(corr_mtx)]
corrplot.mixed(corr_mtx)
```
Unsurprisingly, results for very similar terms tend to have a lot of collinearity:

* `bayesian_decision_analysis` ~ `decision_analysis` = 84%
* `remote_earth_observation` ~ `satellite_earth_observation` = 97%
* `valuation_of_information` ~ `valuation_of_information` = 92%
* `socioeconomic_benefits` ~ `societal_benefits` = 91%
* and 12 other combos with correlation > 0.50
...
pca_rot_df <- data.frame(sbr_faiss_pca$rotation %>% round(4) * 10) %>%
...
sbr_split <- initial_split(pca_df %>%
