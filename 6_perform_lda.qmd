...
Let's pull a subset out of the abstracts for testing various values of $k$, using `perplexity()` scores.  Let's take $k$ values up to about 50 to see if there are any obvious patterns.  
...
The steepest part of the curve ends around $k = 25$, then flattens; if we consider this like a "knee" method, then 25 topics seems like a good place to be: tractable, and max _marginal_ perplexity vs. k.  However, in the vignette for `topicmodels`, on a small corpus (500 docs) all in the same discipline (statistical software), the authors decided on $k=30$, so perhaps a higher $k$ is warranted in our case.

## Running LDA on full corpus

Now let's take the entire abstract set and apply the LDA process, choosing a $k$ and allowing the algorithm to estimate $\alpha$.  Extract the most likely topic for each paper, and extract the top 20 terms for each topic.  Write out as .csv files.
