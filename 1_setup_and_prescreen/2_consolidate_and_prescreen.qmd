---
title: 'Systematic Map: consolidate records and prescreen'
format: 
  html:
    code-fold: true
...
Here we discard duplicated references, keeping the most recent version (if multiple years) that also contains a DOI (if any), and (arbitrarily) the Web of Science record if found in both sources.   Papers in the benchmark set are also removed so they can be flagged separately as screened papers.
...
dupes_dropped <- all_docs_distinct %>% 
  ...
  mutate(doi_present = !is.na(doi)) %>%
  arrange(!doi_present, desc(year), desc(src)) %>%
  # arrange(desc(year), desc(src)) %>%
  slice(1) %>%
  ungroup() %>%
  select(-n, -n_doi, -n_dist_doi, -doi_present) %>%
  select(-first_author, -title_short, -abstr_short) %>%
  filter(!(source_type %in% c('CONF', 'INPROCEEDINGS') & is.na(doi)))
...
write_refs(clean_scopus, format = 'ris',
           file = sprintf(here('_data/refs_ris_out/scopus_clean_%s.ris'), 
                          scopus_date))
write_refs(clean_wos,    format = 'ris',  
           file = sprintf(here('_data/refs_ris_out/wos_clean_%s.ris'), 
                          wos_date))
```
