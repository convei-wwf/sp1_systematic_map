...
bib_raw_fs <- list.files(here('_data/1_refs_raw'), pattern = 'zot.+.bib$', full.names = TRUE)
for(f in bib_raw_fs) {
  # f <- bib_raw_fs[1]
  bib_raw <- read_file(f)
  bib_clean <- str_replace_all(bib_raw, '(\\r)?\\n   ', ' ')
  f_out <- str_replace(f, '1_refs_raw', '2_refs_preprocessed')
...
wos_raw_fs <- list.files(here('_data/1_refs_raw'), pattern = 'wos_.+.bib$', full.names = TRUE)
...
wos_clean <- lapply(wos_raw_fs, 
  ...
  paste0(collapse = '\\n\\n')
# f_out <- here('_data/2_refs_preprocessed', sprintf('wos_%s.bib', wos_date))
...
for(i in 1:n_chunks) {
  ...
  f_out <- here('_data/2_refs_preprocessed', sprintf('wos_%s_%s.bib', i, wos_date))
...
ris_raw_fs <- list.files(here('_data/1_refs_raw'), 
...
out_fstem <- here('_data/2_refs_preprocessed/scopus_%s_%s.ris')
for(i in 1:n_chunks) {
  ### i <- 1
