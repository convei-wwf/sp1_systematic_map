...
```{r}
crit <- 'The context of the study relates to justice, equity, and/or one of the NASA Applied Science themes (Capacity Building, Climate & Resilience, Disasters, Health & Air Quality, Water Resources, Ecological Conservation, Agriculture, Wildland Fires)'

prompt <- c('Here is a criterion: ',
            crit ,
            'Those are the inclusion criteria.  Given the following title and abstract for a document, determine whether the document meets all the inclusion criteria.  If the title and abstract meets all criteria, respond only with "include" and no other text.  If it fails to meet one or more criteria, respond with "exclude", followed by a comma, followed by the letters of the criteria it failed to meet concatenated together.  For example, if the title and abstract failed to meet criteria A and C, you would respond with "exclude,AC" and no other text.  If you are uncertain whether it meets one or more criteria, respond with "uncertain", followed by a comma, followed by the letters of the criteria it failed to meet concatenated together, for example, "uncertain,AC" and no other text.  Here is the title and abstract:'
            )
```

```{r, message = NA}
out_f <- here('_output/societal_benefits/chatgpt_autoscreen1.csv')
  ...
  for(i in 1:nrow(out_df)) {
    ...
    post_body <- list(
      ...
      messages = list(
        ...
        list(role = 'user', content = str_c(prompt, input, collapse = ' '))
      )
    )
