...
files_df <- read_csv(here('_data/societal_benefits/int/fulltext_files.csv')) %>%
  mutate(across(where(is.character), ~str_remove(.x, '^.+sp1_systematic_map/'))) %>%
  mutate(csv_out = here(csv_out))
...
The sleep time might not be sufficient - there are a few "rate_limit_exceeded" errors on smaller papers...
...
out_df <- files_df %>%
  select(-csv_out, -pdf_out, -file) %>%
  mutate(n_tokens = NA, summary = NA, error = NA)
...
if(!file.exists(out_f)) {
  ...
  prompt <- 'Please summarize the methods of this paper.  I am particularly interested in methods used to estimate or quantify the societal benefits of satellite or Earth Observation data.'
...
## API Access - fill in gaps

Let's identify some of those "rate_limit_exceeded" errors and try again.
...
if(!file.exists(out_f)) {
  ...
  for(i in 1:nrow(txt_df)) {
    ...
    n_tokens <- data.frame(t = fulltxt) %>%
      unnest_tokens(output = word, input = t, token = 'words') %>%
      .$word %>% length()
    ...
    if(cum_tokens > 60000) {
      ...
      ### take a nap
      if(wait_time > 0) {
        message('...Pausing to not exceed 60,000 tokens per minute')
        Sys.sleep(wait_time)
      }
      ### reset timer and token count
      cum_tokens <- 0
