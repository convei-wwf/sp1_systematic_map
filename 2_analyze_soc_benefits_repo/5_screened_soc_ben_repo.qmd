...
The 260 unique(?) documents with abstracts represented in the Societal Benefits repository were screened by title and abstract in Colandr.  The results were then full-text screened and tagged.  Here we examine the "includes" and compare them to the tags implemented in the original SBR.
...
sbr_results_df <- read_csv(here('_output/societal_benefits/fulltext_screened.csv')) %>%
  mutate(join_title = tolower(title) %>%
