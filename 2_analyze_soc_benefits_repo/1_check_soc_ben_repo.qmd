...
sbr_results_df <- read_csv(here('_data/4_colandr_screened/colandr_companion_incl_240429.csv')) %>%
  bind_rows(read_csv(here('_data/4_colandr_screened/colandr_companion_excl_240429.csv'))) %>%
  mutate(title = str_to_title(title) %>% stringi::stri_trans_general('Latin-ASCII'),
...
# write_csv(sbr_out_df, here('_data/5a_societal_benefits/sbr_screened_results_240429.csv'))

sbr_out_df <- read_csv(here('_data/5a_societal_benefits/sbr_screened_results_240429.csv'))

sbr_to_zotero <- sbr_out_df %>%
  filter(screening_status == 'included') %>%
  select(source_type, author, year, title, abstract, doi, notes = key) %>%
  as.data.frame()
write_refs(sbr_to_zotero, format = 'bib', file = here('_data/5_bib_to_zotero', '1_bib_to_zotero_sbr.bib'))
```
