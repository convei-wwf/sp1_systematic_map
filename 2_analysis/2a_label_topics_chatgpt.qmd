...
ggplot(top_match_df, aes(x = prob)) +
  geom_histogram() +
  labs(x = 'Highest topic percent match per article',
...
ggplot(top2_match_df, aes(x = prob)) +
  geom_histogram() +
  labs(x = 'Top-two topic percent match per article',
       y = 'Number of articles') +
  theme_minimal()
...
ggplot(topic_top_match, aes(y = label, x = sum_prob)) +
  geom_col() +
  theme(axis.title.y = element_blank()) +
  theme_minimal() +
  labs(x = 'Probability-weighted article count')
```
...
ggplot(topic_singles, aes(y = topic, x = n)) +
  geom_col(fill = hcl.colors(1)) +
  geom_text(aes(label = label), x = 10, color = 'grey50', hjust = 0) +
  theme_minimal() +
