...
Based on this sample analysis, we see low perplexities below $k$ of 10 topics.  However, in the vignette for `topicmodels`, on a small corpus (500 docs) all in the same discipline (statistical software), the authors decided on $k$ of 30, so perhaps a higher $k$ is warranted in our case.  Since we have 8 NASA Applied Science themes, let's go for a smaller $k$, looks like 9 has a slightly lower perplexity than anything else above 3.

## Running LDA on full corpus

Now let's take the entire abstract set and apply the LDA process, choosing a $k$ and allowing the algorithm to estimate $\alpha$.  Extract the most likely topic for each paper, and extract the top 100 terms for each topic, with probability.  Write out as .csv files.
