...
```{r perform LDA on WOS only abstract set}

k_vec <- c(14, 30)

abstr_wos_df <- abstr_df %>%
  filter(str_detect(bibtex_source, 'wos'))
abstr_wos_words_df <- abstr_wos_df %>%
  unnest_tokens(input = text, output = word, token = 'words') %>%
  anti_join(stop_words, by = 'word') %>%
  filter(!str_detect(word, '[0-9]')) %>%
  filter(nchar(word) > 2) %>%
...
for(k in k_vec) {
  topic_outf <- sprintf(here('_output/lda_topic_k%s_wos_only.csv'), k)
  terms_outf <- sprintf(here('_output/lda_terms_k%s_wos_only.csv'), k)
  
  if(any(!file.exists(topic_outf, terms_outf))) {
  
    set.seed(4321)
    
    message('Processing the WOS-only abstract set of ', nrow(abstr_wos_df), 
            ' titles, with k = ', k, '...')
    
    abstr_words_dtm <- cast_dtm(abstr_wos_words_df, 
                                document = doc_id, term = word, value = n)
