---
title: "Phase 4: gather and prepare full texts for included items in classifier set 2b"
author: "O'Hara"
format: 
  html:
    code-fold: true
...
Note that we only screened `r nrow(results_2b_df)` citations out of `r nrow(cls_rd2_refs)` for round 2a.
...
to_zot_f <- here('_data/5_screen_classifier_round2/bib_to_zotero_cls_round2b.bib')
...
```{r}
zot_check <- read_refs(here('_data/5_screen_classifier_round2/bib_from_zotero_cls_round2b.bib')) %>%
...
fulltext_df <- incl_df %>%
  ...
  mutate(phase = 'classifier round 2b',
...
write_csv(fulltext_df, here('_data/screened_fulltext/4b_tmp_classifier_round2b.csv'))
```
