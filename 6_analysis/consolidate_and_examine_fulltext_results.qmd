...
```{r}
fs <- list.files(here('_data/screened_fulltext'), pattern = '^[0-9]_fulltext_.+.csv$', full.names = TRUE)
...
ggplot(method_df, aes(x = 1, y = pct, fill = method)) +
  geom_bar(stat = 'identity', width = 1, alpha = .7) +
  geom_text(aes(label = method, y = lbl_y, angle = lbl_angle, color = lbl_color),
            x = 1.1) +
...
ggsave('methods_pie.png', height = 5, width = 5, dpi = 300)
```
...
ggplot(theme_df, aes(x = 1, y = pct, fill = theme)) +
  geom_bar(stat = 'identity', width = 1, alpha = .7) +
  geom_text(aes(label = theme, y = lbl_y, angle = lbl_angle, color = lbl_color),
            x = 1.1) +
  coord_polar('y', start = 0) +
  scale_fill_manual(values = c(rev(hcl.colors(8)), 'grey50')) +
  scale_color_manual(values = c('white', 'black')) +
...
ggsave('themes_pie.png', height = 5, width = 5, dpi = 300)
```
