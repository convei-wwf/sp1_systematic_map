...
ggsave('fig1_method_by_value.png', height = 4, width = 6, dpi = 300)
...
f2_plot <- ggplot(sba_by_value_df, aes(y = sba, x = n)) +
  geom_col(aes(fill = val_type)) +
  # guides(fill = guide_legend(reverse = TRUE)) +
  geom_text(data = df_hi, aes(label = lbl, x = n_tot), color = 'white',
            hjust = 1, size = 3, nudge_x = -1) +
...
ggsave('fig2_sba_by_value.png', height = 4, width = 6, dpi = 300)

knitr::include_graphics('fig2_sba_by_value.png')
