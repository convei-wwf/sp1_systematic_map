...
write_refs(bench_clean,  format = 'ris',
           file = here('_data/output_for_colandr/bench_clean.ris'))


refs_ready <- dupes_dropped %>%
  select(-first_author, -title, -doc_id, -n) %>%
  select(where(~ any(!is.na(.))))
  
set.seed(42)
clean_sample_for_title_screen <- dupes_dropped %>%
  sample_n(1000) %>%
  ### write_refs can't handle tbl?
  as.data.frame()

write_refs(clean_sample_for_title_screen, format = 'ris',
           file = here('_data/output_for_colandr/sample.ris'))
...
write_refs(clean_scopus, format = 'ris',
           file = sprintf(here('_data/output_for_colandr/scopus_clean_%s.ris'), scopus_date))
write_refs(clean_wos,    format = 'ris',  
           file = sprintf(here('_data/output_for_colandr/wos_clean_%s.ris'), wos_date))
