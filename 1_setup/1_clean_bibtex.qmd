...
wos_raw_fs <- list.files(here('_data/bibtex_raw'), pattern = 'wos_.+.bib$', full.names = TRUE)
...
for(i in 1:n_chunks) {
  ...
  f_out <- here('_data/bibtex_clean', sprintf('wos_%s_%s.bib', i, wos_date))
...
ris_raw_fs <- list.files(here('_data/bibtex_raw'), pattern = 'scop.+.ris$', full.names = TRUE)
...
out_fstem <- here('_data/bibtex_clean/scopus_%s_%s.ris')
for(i in 1:n_chunks) {
  ### i <- 1
