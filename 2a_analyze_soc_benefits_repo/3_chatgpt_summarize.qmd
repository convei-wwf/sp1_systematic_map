...
```{r}
files_df <- read_csv(here('_data/5a_societal_benefits/int/fulltext_files.csv')) # %>%
  # filter(file.mtime(csv_out) > as.Date('2024-08-14'))

csv_fs <- files_df$csv_out

txt_df <- lapply(here(csv_fs), 
                 FUN = function(f) data.table::fread(f) %>% mutate(p = as.character(p))) %>%
  setNames(csv_fs) %>%
  bind_rows(.id = 'file') %>%
  group_by(file) %>%
...
out_f <- here('_output/phase1_societal_benefits/chatgpt_summary.csv')
